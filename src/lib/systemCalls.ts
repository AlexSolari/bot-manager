...
export async function getStorageFiles() {
    return (import.meta.env.MODE == "development"
        ? (await execAsync('(ls -r D:\\!src\\funnyBot\\storage).fullname', { 'shell': 'powershell.exe' })).stdout
        : (await execAsync('sudo find /mnt/dietpi_userdata/funnyBot/storage -name "*.json"')).stdout).split('\n')
        .filter(x => x)
